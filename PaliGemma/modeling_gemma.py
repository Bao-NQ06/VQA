...
class GemmaRMSNorm(nn.Module):
    ...
    def forward(self, x):
        return (self._norm(x.float()) * (1.0 + self.weight.float())).astype(x)
...
class GemmaMLP(nn.Module):
    def __init__(self, config):
        ...
        self.down_proj = nn.Linear(config.intermediate_size, config.intermediate_size, bias=False)
...
def repeat_kv(hidden_states: torch.Tensor, n_rep: int) -> torch.Tensor:
    batch, num_key_value_heads, seq_length, head_dim = hidden_states.shape
    if n_rep == 1:
        return hidden_states
    hidden_states= hidden_states[:, :, None, :, :].expand(batch, num_key_value_heads, n_rep, seq_length, head_dim)
    return hidden_states.reshape(batch, num_key_value_heads * n_rep, seq_length, head_dim)
...
class GemmaRotaryEmbedding(nn.Module):
    def __init__(self, head_dim, max_position_embeddings=2048, base = 10000, device = None):
        ...
        # Calculate the theta according to the formula theta_i = base ^ (-2i/dim) where i is the position and dim //2
    ...
    def forward(self, x ,position_ids, seq_len=None):
        ...
        with torch.autocast(device_type=device_type, enabled=False):
            freqs = (inv_freq_expanded.float() @ position_ids_expanded.flaot()).transpose(1, 2)
...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        cos, sin = self.rotary_emb(value_states, position_ids, seq_length=None)
        ...
        # Multiply the attention weights with the value states: [batch_size, num_heads_Q, seq_length_Q, Seq_len_KV] x [batch_size, num_heads_KV, seq_length_KV, head_dim] -> [batch_size, num_heads_Q, seq_length_Q, head_dim]

        attn_output = torch.matmul(attn_weights, value_states)
        
        if attn_output.size() != (batch_size, self.num_heads, seq_length, self.head_dim):
            raise ValueError(f"Attention output has the wrong shape. Expected {(batch_size, self.num_heads, seq_length, self.head_dim)}, got {attn_output.size()}")
        
        # [batch_size, seq_length_Q, Num_head_Q * head_dim] 
        attn_output = attn_output.transpose(1, 2).contiguous().view(batch_size, seq_length,-1)
        
        # Multiply by W_o. [Batch_size, seq_length_Q, hidden_size]
...
class GemmaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states, _ = self.self_attention(hidden_states=hidden_states, attention_mask=attention_mask, position_ids= position_ids, kv_cache=kv_cache)
...
class PaliGemmaForConditionalGeneration(nn.Module):
    ...
    def _merge_input_ids_with_image_features(
        ...
        # Shape: [batch_size, seq_length]. True for text tokens
        text_mask = (input_ids != self.config.image_token_index) & (input_ids != self.pad_token_id)
        # Shape: [batch_size, seq_length]. True for image tokens
        image_mask = input_ids == self.config.image_token_index
        # Shape: [batch_size, seq_length]. True for padding tokens
    ...
    def forward(
        ...
        assert torch.all(attention_mask == 1), "Attention mask is not supported in this model"
        
        #1. Extra the input embeddings 
        inputs_embeds = self.language_model.get_input_embedding()(input_ids)
        
        #2. Merge the vision and text embeddings
        ...
        #Merge the image and text embeddings
